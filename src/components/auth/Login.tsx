...
import {
  ...
  ArrowRight, Sparkles, Users, CheckCircle2, Lock, Palette,
} from 'lucide-react';
...
export const Login: React.FC = () => {
  ...
  const [persona, setPersona] = useState<UserSession>(DEMO_PERSONAS[0]);
            ...
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, maxHeight: 196, overflowY: 'auto' }}>
                {DEMO_PERSONAS.map(p => {
                  ...
                  return (
                    <button key={p.id} type="button" onClick={() => setPersona(p)} style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '9px 10px', borderRadius: 12, border: `1.5px solid ${sel ? t.accentText : t.border}`, background: sel ? t.badgeBg : 'transparent', cursor: 'pointer', transition: 'all 0.15s' }}>
