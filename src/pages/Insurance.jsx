...
export const InsuranceModule = () => {
    ...
    const filteredClaims = insuranceClaims.filter((claim) => {
        ...
        const matchSearch = claim.patientName.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    return (<div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900/60 p-6 rounded-2xl border border-slate-800/80">
        ...
        <button onClick={() => addToast('Pre-Auth Form Launched', 'Upload insurance policy copy & estimation bill.', 'info')} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-purple-600 hover:bg-purple-500 text-white text-xs font-bold shadow-md shadow-purple-600/20 transition">
      ...
      <div className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800/80 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            {['All', 'Pre-Auth Submitted', 'Pre-Auth Approved', 'Claim Settled'].map((st) => (<button key={st} onClick={() => setSelectedStatus(st)} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition ${selectedStatus === st
          ...
          <div className="relative">
            ...
            <input type="text" placeholder="Search Claim #, Provider..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="bg-slate-950 border border-slate-800 rounded-xl pl-9 pr-3 py-1.5 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-purple-500"/>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-800/60 text-slate-200">
              {filteredClaims.map((claim) => (<tr key={claim.id} className="hover:bg-slate-800/40 transition">
                  ...
                  <td className="py-3 px-3">
                    <button onClick={() => addToast('Documents Verified', `Verified documents for ${claim.claimNo}`, 'info')} className="px-2.5 py-1 rounded bg-slate-800 hover:bg-slate-700 text-slate-200 text-[10px] font-semibold border border-slate-700 transition">
                      Inspect Dossier
                    </button>
                  </td>
                </tr>))}
...
export const Insurance = InsuranceModule;
