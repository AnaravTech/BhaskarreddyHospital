...
export const HousekeepingModule = () => {
    ...
    return (<div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {housekeepingTasks.map((task) => (<div key={task.id} className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800/80 hover:border-slate-700 transition flex flex-col justify-between space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className={`px-2.5 py-0.5 text-[10px] font-bold rounded border ${task.status === 'Completed' || task.status === 'Inspected'
            ...
            <div className="flex items-center justify-between pt-1 text-xs">
              ...
              {task.status !== 'Inspected' && (<button onClick={() => updateHousekeepingStatus(task.id, 'Inspected')} className="px-3 py-1 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-[10px] transition">
...
export const Housekeeping = HousekeepingModule;
