...
import { BedDouble, CheckCircle2, User, ArrowRightLeft, } from 'lucide-react';
export const BedManagementModule = () => {
    ...
    const [sourceBed, setSourceBed] = useState(null);
    ...
    const handleExecuteTransfer = () => {
        ...
        updateBedStatus(targetBed.id, 'Occupied', sourceBed.patientId, sourceBed.patientName, sourceBed.patientUhid);
        addToast('Bed Transfer Completed', `Shifted ${sourceBed.patientName} from ${sourceBed.bedNumber} to ${targetBed.bedNumber}`, 'success');
    ...
    return (<div className="space-y-6 pb-12">
      ...
      <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800/80 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-wrap gap-1">
            {floors.map((f) => (<button key={f} onClick={() => setSelectedFloor(f)} className={`px-3 py-1 rounded-lg text-xs font-semibold transition ${selectedFloor === f
        ...
        {/* Type & Status Filters */}
        <div className="flex items-center gap-3">
          <select value={selectedType} onChange={(e) => setSelectedType(e.target.value)} className="bg-slate-950 border border-slate-800 rounded-xl px-3 py-1.5 text-xs text-slate-200">
            {bedTypes.map((t) => (<option key={t} value={t}>
                {t === 'All' ? 'All Ward Types' : `${t} Wards`}
              </option>))}
          </select>

          <select value={selectedStatus} onChange={(e) => setSelectedStatus(e.target.value)} className="bg-slate-950 border border-slate-800 rounded-xl px-3 py-1.5 text-xs text-slate-200">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {filteredBeds.map((bed) => (<div key={bed.id} className="p-4 rounded-2xl bg-slate-900/90 border border-slate-800/80 hover:border-slate-700 transition flex flex-col justify-between space-y-3 relative group">
            {/* Card Top */}
            ...
            <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800/60 text-xs">
              {bed.status === 'Occupied' ? (<div className="space-y-1">
                ...
                </div>) : (<div className="text-slate-500 italic text-[11px]">
            ...
            <div className="flex items-center justify-between pt-2 border-t border-slate-800/60 text-[10px]">
              ...
              <div className="flex items-center gap-1">
                {bed.status === 'Occupied' && (<button onClick={() => {
                    setSourceBed(bed);
                ...
                {bed.status === 'Cleaning' && (<button onClick={() => updateBedStatus(bed.id, 'Available')} className="px-2 py-1 rounded bg-emerald-500/20 text-emerald-300 font-semibold flex items-center gap-1 transition">
                    <CheckCircle2 className="w-3 h-3"/>
                    <span>Mark Ready</span>
                  </button>)}

                {bed.status === 'Available' && (<select onChange={(e) => updateBedStatus(bed.id, e.target.value)} value={bed.status} className="bg-slate-950 text-slate-300 rounded border border-slate-800 text-[10px] px-1 py-0.5">
      ...
      {/* Bed Transfer Modal */}
      {transferModalOpen && sourceBed && (<div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl space-y-4">
            ...
            <div className="text-xs space-y-3">
              ...
              <div>
                ...
                <select value={targetBedId} onChange={(e) => setTargetBedId(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-slate-100">
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button onClick={() => setTransferModalOpen(false)} className="px-4 py-2 rounded-xl bg-slate-800 text-slate-300">
                  Cancel
                </button>
                <button onClick={handleExecuteTransfer} disabled={!targetBedId} className="px-5 py-2 rounded-xl bg-cyan-600 disabled:opacity-50 text-white font-bold">
                  Confirm Transfer
                </button>
              </div>
            </div>
...
export const WardsBeds = BedManagementModule;
