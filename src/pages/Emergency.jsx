...
export const EmergencyModule = () => {
    ...
    return (<div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900/60 p-6 rounded-2xl border border-slate-800/80">
        ...
        <button onClick={() => setIsModalOpen(true)} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-500 text-white text-xs font-bold shadow-lg shadow-rose-600/30 transition animate-bounce">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="p-5 rounded-2xl bg-slate-900/80 border border-rose-500/40 space-y-3">
          ...
          <div className="space-y-3">
            ...
            .map((emg) => (<div key={emg.id} className="p-4 rounded-xl bg-slate-950 border border-rose-500/30 space-y-2">
        ...
        <div className="p-5 rounded-2xl bg-slate-900/80 border border-amber-500/40 space-y-3">
          ...
          <div className="space-y-3">
            ...
            .map((emg) => (<div key={emg.id} className="p-4 rounded-xl bg-slate-950 border border-amber-500/30 space-y-2">
                  <div className="flex justify-between items-start">
                    <span className="font-bold text-slate-100 text-sm">{emg.patientName}</span>
                  ...
                  {emg.medicoLegalCase && (<span className="px-2 py-0.5 rounded text-[9px] font-bold bg-purple-500/20 text-purple-300 border border-purple-500/30 inline-block">
      ...
      {/* Registration Modal */}
      {isModalOpen && (<div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="w-full max-w-lg bg-slate-900 border border-rose-500/40 rounded-2xl p-6 shadow-2xl space-y-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              <div>
                ...
                <input type="text" placeholder="Patient Name or Unknown/Unidentified Male" value={formData.patientName} onChange={(e) => setFormData({ ...formData, patientName: e.target.value })} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-slate-100" required/>
              </div>

              <div>
                <label className="block text-slate-400 mb-1">Triage Priority</label>
                <select value={formData.triagePriority} onChange={(e) => setFormData({ ...formData, triagePriority: e.target.value })} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-slate-100 font-bold text-rose-400">
              ...
              <div>
                ...
                <input type="text" placeholder="e.g. Chest pain, RTA, Laceration" value={formData.chiefComplaint} onChange={(e) => setFormData({ ...formData, chiefComplaint: e.target.value })} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-slate-100"/>
              </div>

              <div className="flex items-center gap-2">
                <input type="checkbox" id="mlc" checked={formData.medicoLegalCase} onChange={(e) => setFormData({ ...formData, medicoLegalCase: e.target.checked })} className="rounded border-slate-800"/>
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button type="submit" className="px-5 py-2 rounded-xl bg-rose-600 text-white font-bold">
...
export const Emergency = EmergencyModule;
