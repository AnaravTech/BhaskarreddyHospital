...
import { Receipt, Search, Plus, Printer, FileText, Lock, } from 'lucide-react';
export const BillingModule = () => {
    const { invoices, processPayment } = useHospital();
    const [selectedInvoiceId, setSelectedInvoiceId] = useState(invoices[0]?.id || '');
    const [searchQuery, setSearchQuery] = useState('');
    // Payment Processing Split State
    const [newSplitMode, setNewSplitMode] = useState('UPI');
    const [newSplitAmount, setNewSplitAmount] = useState('');
    const [newSplitRef, setNewSplitRef] = useState('');
    const [discountInput, setDiscountInput] = useState('');
    const selectedInvoice = invoices.find((inv) => inv.id === selectedInvoiceId);
    const filteredInvoices = invoices.filter((inv) => inv.invoiceNo.toLowerCase().includes(searchQuery.toLowerCase()) ||
        inv.patientName.toLowerCase().includes(searchQuery.toLowerCase()) ||
        inv.patientUhid.toLowerCase().includes(searchQuery.toLowerCase()));
    const handleAddPaymentSplit = (e) => {
        e.preventDefault();
        if (!selectedInvoice || !newSplitAmount)
            return;
    ...
    return (<div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900/60 p-6 rounded-2xl border border-slate-800/80">
        ...
        <button onClick={() => alert('Printing itemized medical invoice receipt...')} disabled={!selectedInvoice} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 text-xs font-semibold transition">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 p-5 rounded-2xl bg-slate-900/80 border border-slate-800/80 space-y-3">
          ...
          <div className="relative">
            ...
            <input type="text" placeholder="Search Invoice #, Patient..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-9 pr-3 py-1.5 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-cyan-500"/>
          </div>

          <div className="space-y-2 max-h-[600px] overflow-y-auto pr-1">
            {filteredInvoices.map((inv) => {
            const isSelected = inv.id === selectedInvoiceId;
            return (<div key={inv.id} onClick={() => setSelectedInvoiceId(inv.id)} className={`p-3.5 rounded-xl border transition cursor-pointer ${isSelected
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`px-2 py-0.5 text-[9px] font-bold rounded ${inv.paymentStatus === 'Paid'
                  ...
                  <div className="mt-2 flex items-center justify-between">
                    ...
                    <div className="text-right font-mono">
                      ...
                      {inv.dueAmount > 0 && (<div className="text-[10px] text-rose-400 font-bold">Due: ₹{inv.dueAmount}</div>)}
                    </div>
                  </div>
                </div>);
        })}
          ...
          {selectedInvoice ? (<>
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1.5">
                  {selectedInvoice.payments.map((p, idx) => (<div key={idx} className="p-2.5 rounded-xl bg-slate-950 border border-slate-800 flex items-center justify-between text-xs">
                      <div className="flex items-center gap-2">
              ...
              {selectedInvoice.dueAmount > 0 && (<form onSubmit={handleAddPaymentSplit} className="p-4 rounded-xl bg-gradient-to-r from-slate-950 to-slate-900 border border-cyan-500/30 space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                    <div>
                      ...
                      <select value={newSplitMode} onChange={(e) => setNewSplitMode(e.target.value)} className="w-full bg-slate-900 border border-slate-800 rounded-lg px-2.5 py-1.5 text-xs text-slate-100">
                    ...
                    <div>
                      ...
                      <input type="number" placeholder={`Max ₹${selectedInvoice.dueAmount}`} value={newSplitAmount} onChange={(e) => setNewSplitAmount(e.target.value)} className="w-full bg-slate-900 border border-slate-800 rounded-lg px-2.5 py-1.5 text-xs text-slate-100 font-mono" required/>
                    </div>

                    <div>
                      <label className="block text-[10px] text-slate-400 mb-1">Transaction Ref / UTR</label>
                      <input type="text" placeholder="UPI Txn ID or Receipt #" value={newSplitRef} onChange={(e) => setNewSplitRef(e.target.value)} className="w-full bg-slate-900 border border-slate-800 rounded-lg px-2.5 py-1.5 text-xs text-slate-100"/>
                    </div>
                  </div>

                  <button type="submit" className="w-full py-2 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white text-xs font-bold shadow-md transition">
