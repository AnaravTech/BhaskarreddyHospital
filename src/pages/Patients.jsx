...
import { Users, Search, Clock, Shield, Phone, AlertCircle, Activity, ChevronRight, Download, } from 'lucide-react';
    ...
    return (<div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900/60 p-6 rounded-2xl border border-slate-800/80">
        ...
        <div className="relative w-full sm:w-72">
          ...
          <input type="text" placeholder="Search by UHID, Name, or Mobile..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-9 pr-3 py-2 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-cyan-500"/>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 p-5 rounded-2xl bg-slate-900/80 border border-slate-800/80 space-y-3">
          ...
          <div className="space-y-2 max-h-[650px] overflow-y-auto pr-1">
            ...
            return (<div key={pat.id} onClick={() => setSelectedPatient(pat)} className={`p-3.5 rounded-xl border transition cursor-pointer ${isSelected
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`px-2 py-0.5 text-[9px] font-bold rounded ${pat.status === 'Admitted'
        ...
        <div className="lg:col-span-7">
          {selectedPatient ? (<div className="p-6 rounded-2xl bg-slate-900/80 border border-slate-800/80 space-y-6">
              {/* Profile Header */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-800">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => alert(`Downloading Digital EHR Summary PDF for UHID: ${selectedPatient.uhid}`)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold border border-slate-700 transition">
              ...
              {validity && (<div className={`p-3.5 rounded-xl border flex items-center justify-between text-xs ${validity.isValid
                  ...
                  {validity.isValid && (<span className="px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-300 font-bold text-[10px] uppercase">
                      Eligible Free OP
                    </span>)}
                </div>)}
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedPatient.allergies.map((allergy, i) => (<span key={i} className="px-2.5 py-1 rounded-lg text-xs font-medium bg-rose-500/10 text-rose-300 border border-rose-500/30">
                      ⚠️ Allergy: {allergy}
                    </span>))}
                </div>
                ...
                <div className="p-3.5 rounded-xl bg-slate-950/60 border border-slate-800 space-y-1">
                  ...
                  <ul className="list-disc list-inside text-xs text-slate-200 space-y-0.5">
                    {selectedPatient.medicalHistory.map((item, idx) => (<li key={idx}>{item}</li>))}
                  </ul>
                </div>
              </div>
            ...
            </div>) : (<div className="p-12 rounded-2xl bg-slate-900/40 border border-slate-800 text-center text-slate-500 text-xs">
...
export const Patients = PatientManagementModule;
