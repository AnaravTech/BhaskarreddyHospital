...
export const DoctorsModule = () => {
    ...
    return (<div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {doctors.map((doc) => (<div key={doc.id} className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800/80 hover:border-slate-700 transition flex flex-col justify-between space-y-4">
            <div className="flex items-start gap-3">
              <img src={doc.image} alt={doc.name} className="w-14 h-14 rounded-2xl object-cover ring-2 ring-cyan-500/30 shadow-md shrink-0"/>
              <div>
            ...
            <div className="flex items-center justify-between text-[11px] pt-1">
              <div className="flex gap-1">
                {doc.availabilityDays.map((d) => (<span key={d} className="px-1.5 py-0.5 rounded bg-slate-800 text-slate-300 font-mono text-[9px]">
                    {d}
                  </span>))}
              </div>
              <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${doc.status === 'On Duty'
...
export const Doctors = DoctorsModule;
