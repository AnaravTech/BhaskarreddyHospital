...
export const ConsentFormsModule = () => {
    ...
    return (<div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900/60 p-6 rounded-2xl border border-slate-800/80">
        ...
        <button onClick={() => alert('Printing digital consent form document with signature proof...')} disabled={!selectedForm} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-800 text-slate-200 border border-slate-700 text-xs font-semibold hover:bg-slate-700 transition">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 p-5 rounded-2xl bg-slate-900/80 border border-slate-800/80 space-y-3">
          ...
          <div className="space-y-2">
            ...
            return (<div key={form.id} onClick={() => setSelectedFormId(form.id)} className={`p-3.5 rounded-xl border transition cursor-pointer ${isSelected
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`px-2 py-0.5 text-[9px] font-bold rounded ${form.status === 'Signed'
        ...
        <div className="lg:col-span-8 p-6 rounded-2xl bg-slate-900/80 border border-slate-800/80 space-y-6">
          {selectedForm ? (<>
              <div className="flex justify-between items-start pb-4 border-b border-slate-800">
                ...
                <span className={`px-3 py-1 text-xs font-bold rounded-xl border ${selectedForm.status === 'Signed'
              ...
              {selectedForm.status === 'Signed' ? (<div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30 space-y-2 text-xs">
                ...
                </div>) : (<form onSubmit={handleSign} className="p-4 rounded-xl bg-slate-950 border border-slate-800 space-y-3 text-xs">
                  ...
                  <div>
                    ...
                    <input type="text" placeholder="e.g. Prashanth Goud (Son)" value={signerName} onChange={(e) => setSignerName(e.target.value)} className="w-full bg-slate-900 border border-slate-800 rounded-xl px-3 py-2 text-slate-100" required/>
                  </div>
                  ...
                  <button type="submit" className="w-full py-2.5 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white font-bold transition shadow-md">
...
export const ConsentForms = ConsentFormsModule;
export default ConsentFormsModule;
