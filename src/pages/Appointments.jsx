...
import { CalendarCheck, Clock, MessageSquare, Search, Plus, Zap, } from 'lucide-react';
export const AppointmentModule = () => {
    ...
    const [bookingModel, setBookingModel] = useState('Premium Slot');
    ...
    const filteredAppointments = appointments.filter((apt) => {
        const matchesModel = activeTab === 'all'
            ? true
            : activeTab === 'premium'
    ...
    return (<div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900/60 p-6 rounded-2xl border border-slate-800/80">
        ...
        <button onClick={() => setIsBookingModalOpen(true)} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white text-xs font-bold shadow-md shadow-cyan-600/20 transition">
      ...
      <div className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800/80 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          {/* Model Filter Pills */}
          <div className="flex items-center gap-2">
            <button onClick={() => setActiveTab('all')} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition ${activeTab === 'all' ? 'bg-cyan-600 text-white' : 'bg-slate-950 text-slate-400 border border-slate-800'}`}>
              All Appointments ({appointments.length})
            </button>
            <button onClick={() => setActiveTab('premium')} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition ${activeTab === 'premium' ? 'bg-purple-600 text-white' : 'bg-slate-950 text-slate-400 border border-slate-800'}`}>
              Premium Slots ({appointments.filter((a) => a.model === 'Premium Slot').length})
            </button>
            <button onClick={() => setActiveTab('normal')} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition ${activeTab === 'normal' ? 'bg-cyan-600 text-white' : 'bg-slate-950 text-slate-400 border border-slate-800'}`}>
          ...
          <div className="flex items-center gap-2">
            <select value={selectedDoctorId} onChange={(e) => setSelectedDoctorId(e.target.value)} className="bg-slate-950 border border-slate-800 rounded-xl px-3 py-1.5 text-xs text-slate-200">
            ...
            <div className="relative">
              ...
              <input type="text" placeholder="Search patient, token..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="bg-slate-950 border border-slate-800 rounded-xl pl-9 pr-3 py-1.5 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-cyan-500"/>
            </div>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-800/60 text-slate-200">
              {filteredAppointments.map((apt) => (<tr key={apt.id} className="hover:bg-slate-800/40 transition">
                  ...
                  <td className="py-3 px-3">
                    <span className={`px-2 py-0.5 text-[10px] font-bold rounded ${apt.model === 'Premium Slot'
                ...
                : 'bg-slate-800 text-slate-300'}`}>
                  ...
                  <td className="py-3 px-3">
                    {apt.isFollowUp ? (<span className="px-2 py-0.5 text-[10px] font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 rounded">
                        15-Day OP Free
                      </span>) : (<span className="font-extrabold text-slate-100">₹{apt.fee}</span>)}
                  </td>
                  <td className="py-3 px-3">
                    <span className={`px-2 py-0.5 text-[10px] font-bold rounded ${apt.status === 'Checked-In'
      ...
      {/* Booking Modal */}
      {isBookingModalOpen && (<div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl space-y-4">
            ...
            <form onSubmit={handleBookingSubmit} className="space-y-4 text-xs">
              <div>
                ...
                <select value={bookingPatientId} onChange={(e) => setBookingPatientId(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-slate-100">
              ...
              <div>
                ...
                <select value={bookingDoctorId} onChange={(e) => setBookingDoctorId(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-slate-100">
                  {doctors.map((d) => (<option key={d.id} value={d.id}>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button type="button" onClick={() => setBookingModel('Premium Slot')} className={`py-2 rounded-xl border text-xs font-bold ${bookingModel === 'Premium Slot'
                ? 'bg-purple-500/20 border-purple-500 text-purple-300'
                : 'bg-slate-950 border-slate-800 text-slate-400'}`}>
                    Premium Slot (Fixed)
                  </button>
                  <button type="button" onClick={() => setBookingModel('Normal Queue')} className={`py-2 rounded-xl border text-xs font-bold ${bookingModel === 'Normal Queue'
              ...
              {bookingModel === 'Premium Slot' && (<div>
                  <label className="block text-slate-400 mb-1">Choose Fixed 10-Min Slot</label>
                  <div className="grid grid-cols-4 gap-2">
                    {timeSlots.map((slot) => (<button key={slot} type="button" onClick={() => setBookingSlot(slot)} className={`py-1.5 rounded-lg border text-[11px] font-mono font-semibold ${bookingSlot === slot
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button type="button" onClick={() => setIsBookingModalOpen(false)} className="px-4 py-2 rounded-xl bg-slate-800 text-slate-300">
                  Cancel
                </button>
                <button type="submit" className="px-5 py-2 rounded-xl bg-cyan-600 text-white font-bold">
                  Confirm Booking
                </button>
              </div>
            </form>
...
export const Appointments = AppointmentModule;
