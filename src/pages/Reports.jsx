...
export const ReportsModule = () => {
    ...
    return (<div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900/60 p-6 rounded-2xl border border-slate-800/80">
        ...
        <button onClick={() => handleExportCSV('Full Hospital Operations Audit')} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white text-xs font-bold shadow-md transition">
      ...
      <div className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800/80 space-y-4">
        ...
        <div className="space-y-2">
          {reportItems.map((rpt, idx) => (<div key={idx} className="p-4 rounded-xl bg-slate-950 border border-slate-800 flex items-center justify-between gap-4 hover:border-slate-700 transition">
              <div>
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => handleExportCSV(rpt.title)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-cyan-300 text-xs font-semibold border border-slate-700 transition">
