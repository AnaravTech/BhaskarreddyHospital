...
export const PublicWebsite: React.FC = () => {
  ...
  // WOW 2: Doctor Finder State
  const [docSearchQuery, setDocSearchQuery] = useState('');
  const [selectedDeptFilter, setSelectedDeptFilter] = useState('All');

  // WOW 3: Booking Wizard State
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans selection:bg-cyan-500 selection:text-white pb-20">
      ...
      {/* Hero Section & WOW Factor 1 Stats */}
      {activeTab === 'home' && (
        <>
          {/* Hero Banner */}
          ...
          <section className="px-4 md:px-12 py-10 bg-slate-900/80 border-y border-slate-800/80">
            <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
              <div className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800 space-y-1">
                <div className="text-3xl md:text-4xl font-black text-cyan-400 font-mono">250,000+</div>
                <div className="text-xs text-slate-400 font-medium">Patients Treated</div>
              </div>
              <div className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800 space-y-1">
                <div className="text-3xl md:text-4xl font-black text-emerald-400 font-mono">25+</div>
                <div className="text-xs text-slate-400 font-medium">Years of Medical Excellence</div>
              </div>
              <div className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800 space-y-1">
                <div className="text-3xl md:text-4xl font-black text-purple-400 font-mono">45+</div>
                <div className="text-xs text-slate-400 font-medium">Senior Specialists & Doctors</div>
              </div>
              <div className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800 space-y-1">
                <div className="text-3xl md:text-4xl font-black text-amber-400 font-mono">50,000+</div>
          ...
          <section className="px-4 md:px-12 py-16 bg-slate-900/60 border-y border-slate-800/80">
            <div className="max-w-7xl mx-auto space-y-8">
              <div className="text-center space-y-2">
                <span className="px-3 py-1 rounded-full text-xs font-semibold bg-cyan-500/10 text-cyan-400 border border-cyan-500/30">
                  WOW Factor 4
                </span>
                <h2 className="text-3xl font-black text-white">Interactive Department Explorer</h2>
                <p className="text-xs text-slate-400">
                  Hover over specialty divisions to discover procedures, facilities, and key consultants.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {departments.map((dept) => (
                  <div
                    key={dept.id}
                    className="p-6 rounded-2xl bg-slate-950 border border-slate-800 hover:border-cyan-500/60 hover:bg-slate-900/90 transition group space-y-4"
                  >
                    <div className="flex items-center justify-between">
                      <h3 className="text-base font-extrabold text-white group-hover:text-cyan-400 transition">
                        {dept.name}
                      </h3>
                      <span className="font-mono text-[10px] text-cyan-400 font-bold px-2 py-0.5 bg-cyan-500/10 rounded">
                        {dept.code}
                      </span>
                    </div>
                    ...
                    <div className="space-y-1">
                      <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Key Procedures & Services:</div>
                      <div className="flex flex-wrap gap-1">
                        {dept.services?.map((srv) => (
                          <span key={srv} className="px-2 py-0.5 rounded bg-slate-900 text-slate-300 text-[10px] border border-slate-800">
                            {srv}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="pt-2 border-t border-slate-800 flex justify-between items-center text-xs">
                      <span className="text-slate-400">Head: {dept.headDoctor.split(',')[0]}</span>
                      <button
                        onClick={() => {
                          setIsBookingModalOpen(true);
                          setBookingStep(1);
          ...
          {/* WOW Factor 5: Hospital Journey & Milestones Timeline */}
          <section className="px-4 md:px-12 py-16 max-w-7xl mx-auto space-y-8">
            <div className="text-center space-y-2">
              <span className="px-3 py-1 rounded-full text-xs font-semibold bg-cyan-500/10 text-cyan-400 border border-cyan-500/30">
                WOW Factor 5
              </span>
              <h2 className="text-3xl font-black text-white">Hospital Journey & Milestones</h2>
              <p className="text-xs text-slate-400">Over 25 years of relentless healthcare innovation</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-xs">
              <div className="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-2">
                <div className="font-mono font-black text-cyan-400 text-lg">1998</div>
                <div className="font-bold text-slate-100">Inception of Bhaskar Reddy Hospital</div>
                <p className="text-slate-400 text-[11px]">Started as a 50-bed community hospital in Gachibowli.</p>
              </div>

              <div className="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-2">
                <div className="font-mono font-black text-blue-400 text-lg">2008</div>
                <div className="font-bold text-slate-100">24/7 Cardiac Cath Lab Wing</div>
                <p className="text-slate-400 text-[11px]">Launched dedicated interventional cardiology unit.</p>
              </div>

              <div className="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-2">
                <div className="font-mono font-black text-purple-400 text-lg">2018</div>
                <div className="font-bold text-slate-100">Robotic Joint Replacement Suite</div>
                <p className="text-slate-400 text-[11px]">Pioneered robotic total knee replacement in Hyderabad.</p>
              </div>

              <div className="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-2">
                <div className="font-mono font-black text-emerald-400 text-lg">2026</div>
                <div className="font-bold text-slate-100">Anarav AI Hospital Operating System</div>
                <p className="text-slate-400 text-[11px]">Deployed digital health platform for zero-latency care.</p>
              </div>
            </div>
          </section>
          ...
          <section className="px-4 md:px-12 py-16 bg-slate-900/60 border-y border-slate-800/80">
            <div className="max-w-7xl mx-auto space-y-8">
              <div className="text-center space-y-2">
                <h2 className="text-3xl font-black text-white">Preventive Health Checkup Packages</h2>
                <p className="text-xs text-slate-400">Comprehensive health screenings for peace of mind</p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {healthPackages.map((pkg) => (
                  <div
                    key={pkg.id}
                    className={`p-6 rounded-2xl bg-slate-950 border transition flex flex-col justify-between space-y-4 ${
                      pkg.isPopular ? 'border-cyan-500 ring-1 ring-cyan-500/50 shadow-xl' : 'border-slate-800'
                    }`}
                  >
                    <div>
                      {pkg.isPopular && (
                        <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-cyan-500 text-slate-950 uppercase tracking-wider mb-2 inline-block">
                          Most Popular
                        </span>
                      )}
                      <h3 className="text-lg font-extrabold text-white">{pkg.title}</h3>
                      <div className="text-xs text-slate-400 mt-1">{pkg.recommendedFor}</div>
      ...
      {/* WOW Factor 7: Floating Emergency Action Banner */}
      ...
      {/* WOW Factor 3: Smart Appointment Booking Modal Wizard */}
      {isBookingModalOpen && (
